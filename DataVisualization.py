...
fig.suptitle("Weather data - "+now.strftime("%B %Y"))
axs[0].plot(DateTime, Temperature, color="C0")
axs[0].set_ylabel(header[1])
axs[0].grid()
axs[1].plot(DateTime, Humidity, color="C1")
axs[1].set_ylabel(header[2])
axs[1].set_ylim(0, 100)
axs[1].grid()
axs[2].plot(DateTime, Rain, color="C4")
axs[2].set_ylabel(header[6])
axs[2].grid()
axs[2].set_xlim(DateTime[0], DateTime[-1])
axs[2].tick_params(axis='x', labelrotation=45)
plt.tight_layout()
plt.savefig("/home/veit/Dokumente/OutdoorWeatherStation/Visualization/"+now.strftime("%Y-%m")+"_THR.svg")

fig, axs = plt.subplots(1, 2, sharey='row', figsize=(7,3.5))
fig.suptitle("Wind speed histograms - "+now.strftime("%B %Y"))
...
axs[1].grid(axis='y')
plt.tight_layout()
plt.savefig("/home/veit/Dokumente/OutdoorWeatherStation/Visualization/"+now.strftime("%Y-%m")+"_Wind.svg")
