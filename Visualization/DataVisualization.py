...
tMin = datetime(2023,10,1)
tMax = datetime(2023,10,31,23,59,59,9999)

keep = []
for i in range(len(TCheck)-1):
    if TCheck[i] >= tMin and TCheck[i] <= tMax:
        if i == 0:
            keep.append(i)
...
plt.title(tMin.strftime("%B %d, %Y")+" - "+tMax.strftime("%B %d, %Y"))
